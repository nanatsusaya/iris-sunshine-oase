...
| **Currently pinned** | `typescript ~5.9.3`; versions `>=7` are ignored. `@astrojs/check@0.9.9` peers on `^5.0.0 \|\| ^6.0.0`. |
...
TypeScript 6, which *is* inside the peer range, still arrives as a normal pull request.
